...
| Command | When |
|---|---|
| `mac` | The shell banner (or the user) flagged drift — refresh the check, summarise what's pending, dispatch the right fix. The menu offers: review-and-apply (diff shown, one confirm), guided backup of locally-edited files (per-file apply / re-add / skip, with template and encrypted sources routed safely), and per-package adopt/uninstall for brew extras. |
